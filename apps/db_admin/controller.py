...
class GetUserMenus(XAuth):
    ...
    def take_on(self, ctx:DefaultContext):
        ...
        user=[dict(label='User Menu', href=ctx.URL('index'))]
        admin=[dict(label='Admin Menu', href=ctx.URL('index'))]
...
def action(ctx: Context):
    ...
    print('CDB is ', cdb) 
    db = ctx.ask(cdb)
    print('Action is ', action, 'Query is ', query )
